...
//  println(part1Answer)
  ...
  def runChessPart2(input: String, passwordMap: Map[Int, Char], digit: Int): String = {
    val md5Hex = convertToMd5Hex(input + digit.toString)
    val newPassword = if (md5Hex.startsWith("00000") && !passwordMap.contains(md5Hex.charAt(5).toInt)) passwordMap ++ Map(md5Hex.charAt(5).toInt -> md5Hex.charAt(6)) else passwordMap
  ...
  val part2Answer = runChessPart2("abc", Map.empty[Int, Char], 0)
  println(part2Answer)
//  returns 05ce3f99 :(
